  ...
  html.querySelectorAll('[data-action="pushRoll"]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const templateData = {
        ...
        extraDice: (newResult.extraResults ?? []).map(v => ({ value: v, success: v === 6 })),
        baseCount: newResult.baseResults.length,
        skillCount: newResult.skillResults.length,
        gearCount: newResult.gearResults.length,
        extraCount: (newResult.extraResults ?? []).length,
        totalPool: newResult.pool.totalPool,
